...
签名 hook 从 rootfs 的 `/var/lib/mipad2/secureboot/MOK.key`（`0600`）读取私钥，
不再复制到 FAT ESP 或刷机包的散文件目录。**这只是位置调整，不是私钥保护或轮换**：
发布的 `xiaomi-latte-rootfs.img` 含有完整 rootfs，下载者可挂载 `@` 子卷并提取该文件，
文件权限 `0600` 不提供对镜像获取者的保密性；该旧私钥也已出现在公开仓库历史中。
当前镜像仍仅适用于已接受该项目证书的设备测试，不应据此宣称信任链安全。
完整修复需在确认 Mi Pad 2 固件接受方式后，生成/登记设备专用证书、改造签名与内核升级流程，
并撤销旧证书；该步骤需在设备旁验证后进行。
